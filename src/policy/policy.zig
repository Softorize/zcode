...
pub const Policy = struct {
    ...
    pub fn classifyTool(self: *const Policy, tool_name: []const u8, args: []const u8) types.RiskTier {
        ...
        if (matchesToolName(tool_name, &.{
            ...
            "GitLog",             "git_log",           "task_poll",
            "JsonQuery",          "json_query",        "mcp_servers_list",
...
fn isKnownToolName(name: []const u8) bool {
    if (matchesToolName(name, &.{
        ...
        "task_poll",         "Skill",          "skill",
        "Command",           "command",        "AskUserQuestion",
        "ask_user_question", "EnterPlanMode",  "enter_plan_mode",
        "ExitPlanMode",      "exit_plan_mode", "TeamCreate",
        "team_create",       "TeamDelete",     "team_delete",
        "SendMessage",       "send_message",   "Move",
        "move",              "Copy",           "copy",
        "Delete",            "delete",         "ListDir",
        "list_dir",          "Stat",           "stat",
        "RunTests",          "run_tests",      "OpenPR",
        "open_pr",           "GitCommit",      "git_commit",
        "GitDiff",           "git_diff",       "GitLog",
        "git_log",           "HttpRequest",    "http_request",
        "JsonQuery",         "json_query",     "Config",
        "config",            "network",
    })) return true;
