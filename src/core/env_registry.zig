...
pub const entries = [_]EnvEntry{
    ...
    .{ .name = "ZCODE_BG_INITIAL_PROMPT", .category = .zcode_config, .description = "Queued [prompt] a /background (or /bg) invocation forwards to the detached --resume child; the child answers it once headlessly (no stdin read) before idling", .sensitive = false },
