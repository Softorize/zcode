...
pub fn replCommandCallback(ctx: *anyopaque, allocator: std.mem.Allocator, command_raw: []const u8) !?[]u8 {
    ...
    // bg_cmds.cmdKill (SIGTERM + registry-file delete only; it never touches
    ...
    //
    // commands-03: a second `/security-review` arm used to live here, but it
    // was provably dead code -- the earlier arm at "/security-review" /
    // "/security_review" (see review_flow.buildSecurityReviewPrompt above)
    // always matches first in this same function, so this branch could never
    // execute. Removed rather than merged: the surviving arm already matches
    // the reference's bundled security-review plugin-skill.
...
test "commands-13: /stop <pid> terminates a registered background session" {
    ...
    try testing.expect(out != null);
    defer allocator.free(out.?);
    try testing.expect(std.mem.indexOf(u8, out.?, "killed pid=") != null);

    // Registry file is gone; transcript/worktree are untouched by cmdKill by
    // construction (it never touches either).
    const dir = try session_registry_mod.registryDir(allocator);
    defer allocator.free(dir);
    const file_path = try std.fmt.allocPrint(allocator, "{s}/{d}.json", .{ dir, pid });
    defer allocator.free(file_path);
    try testing.expectError(error.FileNotFound, std.Io.Dir.cwd().statFile(rt.io, file_path, .{}));
